...
class OpenLibrary {
    ...
    public function searchBook(string $search) {
        $response = $this->client->get('search.json', [
            'query' => [
                'q' => $search,
                'sort' => 'rating',
                'fields' => 'key,title,cover_i,author_key,author_name,first_publish_year,ratings_average'
            ]
        ]);
        $books = [];
     ...
     * @return Book|null
     */
    public function findBook(string $key): Book|null {
        $books = $this->searchBook("key:/works/$key");
     ...
     * @return Book[]
     */
    public function booksForAuthor(string $key): array {
        return $this->searchBook("author_key:$key");
    }
